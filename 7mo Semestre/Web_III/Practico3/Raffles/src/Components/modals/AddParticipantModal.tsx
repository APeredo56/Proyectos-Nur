...
const AddParticipantModal = ({ isOpen, handleOpen, onConfirm, participants, maxTickets }: Props) => {
    ...
    const submit = () => {
        setAddError("");
        if (participants.length + newParticipants.length > maxTickets) {
            setAddError("Los participantes seleccionados superan la cantidad maxima de tickets");
            return;
        }
    ...
    return (<Dialog open={isOpen} handler={onConfirm} className="bg-dark-blue border">
        ...
        <DialogBody className="text-white overflow-y-scroll scroll-container max-h-[50vh]">
            {addError!== "" && <Typography color="red" className="text-center">{addError}</Typography>}
